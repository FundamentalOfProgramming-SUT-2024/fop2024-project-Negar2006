...
void save_score(const char *username, int score) {
    ...
    if (!found) {
        fprintf(temp, "%s %d\n", username, score); // اضافه کردن کاربر جدید
    }
...
void pre_game_menu(const char *username) {
    ...
    while (1) {
        ...
        mvprintw(13, 10, "3. Back to Main Menu");
        mvprintw(14,10,"4. Profile: ");
        mvprintw(15,10,"5. Setting");
        mvprintw(16,10,"6. Score Board: ");
        ...
        switch (choice) {
            ...
            case 2:
                clear();
                mvprintw(10, 10, "Continuing the previous game...");
                getch();
                break;
            case 3:
                return;
...
void welcome_screen() {
    ...
    mvprintw(15, 10, "Select an option: ");
}
void main_menu() {
    int choice;
...
void game_screen(){
    ...
    handle_input();
    if(check_game_over()){
        mvprintw(15,20,"Game Over! You reached the end:))");
        refresh();
    }
    getch();
}
